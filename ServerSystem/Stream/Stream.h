...
public:
	...
	void SetupBuffer(SIZE_T bufferSize, BYTE bigEndian, BYTE autoBuffer = TRUE)
	{
		this->buffer = bufferSize > 0 ? (BYTE *)malloc(bufferSize) : nullptr;
		this->bufferSize = bufferSize;
		this->idx = 0;
		this->bigEndian = bigEndian;
		this->autoBuffer = autoBuffer;

		this->allocatedInst = TRUE;
	...
	/* Initialize the ByteStreamBuffer like the constructors do. */
	void SetupBuffer(BYTE *buffer, SIZE_T bufferSize, BYTE bigEndian, BYTE autoBuffer = TRUE)
	{
		this->buffer = buffer;
		this->bufferSize = bufferSize;
		this->idx = 0;
...
class SSAPI ByteStreamWriter : private ByteStreamBuffer
{
public:
	ByteStreamWriter();
	/*
	...
	*/
	...
	void Initialize(SIZE_T bufferSize, BYTE bigEndian = FALSE, BYTE autoBuffer = TRUE) { this->SetupBuffer(bufferSize, bigEndian, autoBuffer); }
...
class SSAPI ByteStreamReader : private ByteStreamBuffer
{
public:
	ByteStreamReader();
	...
	void Initialize(BYTE *buffer, SIZE_T bufferSize, BYTE bigEndian) { this->SetupBuffer(buffer, bufferSize, bigEndian, FALSE /* read-only, no buffer-sizing needed (or used) */); }

	/* Initialize the ByteStreamReader like the constructors do. */
	template<typename buffer_T>
	void Initialize(buffer_T buffer, SIZE_T bufferSize, BYTE bigEndian) { this->SetupBuffer((BYTE *)buffer, bufferSize, bigEndian, FALSE); }

	/* Initialize the ByteStreamReader like the constructors do. */
	template<typename buffer_T>
	void Initialize(buffer_T buffer, BYTE bigEndian) { this->SetupBuffer((BYTE *)buffer, sizeof(buffer), bigEndian, FALSE); }
